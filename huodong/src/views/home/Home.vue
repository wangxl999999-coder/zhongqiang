  ...
  <div class="min-h-screen bg-gray-50 pb-6 lg:pb-0">
    ...
    <div class="max-w-3xl mx-auto p-4 space-y-4">
      <div 
        ...
        class="card cursor-pointer hover:shadow-md transition-shadow lg:flex lg:gap-4"
      >
        <img :src="activity.cover" :alt="activity.title" class="w-full h-40 lg:w-48 lg:h-32 object-cover rounded-lg mb-3 lg:mb-0" />
        <div class="flex-1">
...
const filteredActivities = computed(() => {
  if (activeCategory.value === 0) return dataStore.allActivities
  const catName = categories.find(c => c.id === activeCategory.value)?.name
  return dataStore.allActivities.filter(a => a.tags.includes(catName))
})
</script>
